...
const { getService, getEffectiveRateLimit, userHasAccess } = require('./services/catalogServiceHelper');
...
const dynamicRateLimiter = async (req, res, next) => {
  try {
    ...
    const maxRequests = await getEffectiveRateLimit(tenantId, user.id, service.id) || 100;
